...
func (order *OrderStruct) Route(EngineRouter *gin.Engine) {
	...
	orderRoutes.Use()
	{
		...
		orderRoutes.POST("/delete", order.DeleteOrder)              //删除订单接口
		orderRoutes.POST("/finishorder", order.FinishOrder)         //确认收货接口
		orderRoutes.POST("/orderdetailpage", order.OrderDetailPage) //订单详情接口
	}
}
